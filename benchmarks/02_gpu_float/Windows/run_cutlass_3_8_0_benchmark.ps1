...
.DESCRIPTION
Runs the CUTLASS profiler built by build_cutlass_3_8_0.ps1 for the local
FP32/TF32 GEMM benchmark subset. Results are written under runs/ with raw
stdout logs, CUTLASS CSV files, a merged summary CSV, command lines, basic
system metadata, and a short Markdown report.
#>
...
function Resolve-CutlassProfiler {
    ...
    throw "cutlass_profiler.exe not found. Build first with .\build_cutlass_3_8_0.ps1, or pass -BuildDir / -ProfilerExe. Checked:`n  $candidateText"
}
...
function New-CutlassReport {
    ...
    foreach ($group in ($perRepeatBest | Group-Object -Property Key | Sort-Object Name)) {
        ...
        $operationText = $best.Operation -replace '\|', '/'
...
$cases = @()
if ($precisionSet.ContainsKey("ALL") -or $precisionSet.ContainsKey("FP32")) {
...
if ($precisionSet.ContainsKey("ALL") -or $precisionSet.ContainsKey("TF32")) {
    $cases += [pscustomobject]@{
        Name = "tf32_tensorop"
        Precision = "TF32"
...
if ($cases.Count -eq 0) {
    throw "No benchmark cases selected. Use -Precisions FP32,TF32 or -Precisions All."
}
if ($RepeatCount -le 0) {
...
New-Item -ItemType Directory -Path $rawDir, $csvDir, $metadataDir -Force | Out-Null
